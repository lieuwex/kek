...
(function() {
  ...
  hangman = (function() {
    function hangman() {
      this.invite = __bind(this.invite, this);
    }
    ...
    hangman.prototype.join = function(bot, out, isPublic, from, to, command, params, message, where) {
      var lobby, player, timeOut;
      player = {
        name: from,
        score: 0,
        triesLeft: 5
      ...
      if ((lobby = hangman.lobbys[to]) != null) {
        if (lobby.started) {
          return bot.currentClient.notice(from, "Game already started.");
        } else if (_.any(lobby.players, function(p) {
          return p.name === from;
        })) {
          return bot.currentClient.notice(from, "You've already joined this game.");
        } else {
          lobby.players.push(player);
          bot.currentClient.say(to, "" + from + " joined. Game starts in " + lobby.secondsLeft + " seconds. Join with @hangman join");
          if (lobby.players.length === 2) {
            return timeOut = setInterval((function() {
              if (--lobby.secondsLeft === 0) {
        ...
        lobby = hangman.lobbys[to] = {
          ...
          started: false,
          guesses: [],
          secondsLeft: SECONDS_CAP,
          _onMoveId: -1,
          onMove: function() {
            return this.players[this._onMoveId % this.players.length];
          },
          filteredWord: function() {
            var char, result, _i, _len, _ref;
            result = "";
            _ref = this.word;
            for (_i = 0, _len = _ref.length; _i < _len; _i++) {
              char = _ref[_i];
              result += _.contains(this.guesses, char.toUpperCase()) ? char : "_";
              result += " ";
            }
            return result;
          },
          ...
          nextTurn: function() {
            ...
            return this._currentTimeout = setTimeout(((function(_this) {
              return function() {
                var _ref;
                if (((_ref = _this.onMove()) != null ? _ref.name : void 0) != null) {
                  bot.currentClient.say(to, "The move of " + (_this.onMove().name) + " took too long. Moving to next person.");
                }
                return _this.nextTurn();
              };
            })(this)), MOVE_TIME_LIMIT * 1000);
          }
        };
        return bot.currentClient.say(to, "Lobby created by " + from + ". Join with @hangman join");
      }
    };

    hangman.prototype.guess = function(bot, out, isPublic, from, to, command, params, message) {
      var guess, lobby, player;
      if ((lobby = hangman.lobbys[to]) != null) {
        if ((player = _.find(lobby.players, function(p) {
          return p.name === from;
        })) != null) {
          guess = params[0].toUpperCase();
          if (guess.length === 1) {
            if (lobby.onMove().name !== from) {
              return bot.currentClient.notice(from, "You're not on move!");
            } else {
              if (_.contains(lobby.guesses, guess)) {
                return bot.currentClient.say(to, "" + guess + " is already chosen. Expecting new guess.");
              } else {
                ...
                if (_.contains(lobby.word.toUpperCase(), guess)) {
                  ...
                  if (--player.triesLeft === 0) {
                    ...
                    if (((lobby = hangman.lobbys[to]) != null) && ((player = _.find(lobby.players, function(p) {
                      return p.name === from;
                    })) != null)) {
                      ...
                      if (lobby != null) {
                        if (typeof lobby.nextTurn === "function") {
                          lobby.nextTurn();
                        }
                      }
                    }
          ...
          } else if (lobby.word.toUpperCase() === guess) {
            bot.currentClient.say(to, "" + from + " guessed the right word: " + lobby.word + "!");
            player.score += 3;
            clearTimeout(lobby._currentTimeout);
            delete hangman.lobbys[to];
            lobby = null;
          } else if (lobby.word.length !== guess.length) {
            ...
            if (--player.triesLeft === 0) {
              ...
              bot.currentClient.say(to, "nope. " + player.triesLeft + " " + (player.triesLeft === 1 ? "try" : "tries") + " left. Expecting new guess.");
            }
            bot.currentClient.say(to, lobby.filteredWord());
    ...
    hangman.prototype.leave = function(bot, out, isPublic, from, to, command, params, message) {
      var lobby, player;
      if (((lobby = hangman.lobbys[to]) != null) && ((player = _.find(lobby.players, function(p) {
        return p.name === from;
      })) != null)) {
    ...
    hangman.prototype.skip = function(bot, out, isPublic, from, to, command, params, message) {
      var lobby, player;
      if ((lobby = hangman.lobbys[to]) != null) {
        if ((player = _.find(lobby.players, function(p) {
          return p.name === from;
    ...
    hangman.prototype.kick = function(bot, out, isPublic, from, to, command, params, message) {
      var lobby, player;
      if ((lobby = hangman.lobbys[to]) != null) {
        if (lobby.admin.name === from) {
          player = _.find(lobby.players, function(p) {
            return p.name.toLowerCase() === params[0].toLowerCase();
          });
          if (player != null) {
            bot.currentClient.say(to, "" + from + " kicked " + player.name + " from hangman.");
            return _.remove(lobby.players, player);
          } else {
            return bot.currentClient.say(to, "No player with the name " + params[0] + " found in the lobby.");
          }
        } else {
          return bot.currentClient.say(to, "You're not the admin of this lobby.");
        }
      } else {
        return bot.currentClient.say(to, "No game in progress, use @hangman join to create and join a new hangman game.");
      }
    ...
    hangman.prototype.invite = function(bot, out, isPublic, from, to, command, params, message) {
      var lobby, name;
      ...
      if (!(((lobby = hangman.lobbys[to]) != null) && (_.find(lobby.players, function(p) {
        return p.name === from;
      }) != null))) {
    ...
    hangman.prototype.acceptInvite = function(bot, out, isPublic, from, to, command, params, message) {
      var name, val;
      name = from.toLowerCase();
      if ((val = hangman.invitations[name]) != null) {
        out("You joined the hangman game over at " + val + ", now part that channel if you haven't yet!");
        return hangman.invitations[name] = null;
      } else {
        return out("You aren't invited yet. :(");
      }
    };
